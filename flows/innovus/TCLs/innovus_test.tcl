#TEST RUN SCRIPT FOR INNOVUS
set REPORTS_DIR /home/p/paschalk/Desktop/I2C_Memory/flows/innovus/Innovus_Results/Test
set SAVES_DIR  /home/p/paschalk/Desktop/I2C_Memory/flows/innovus/Innovus_Results/Test/Saved_Designs
set MODE Test
set SCAN_DEF_DIR /home/p/paschalk/Desktop/I2C_Memory/flows/genus/Genus_Results/Test/Genus_Out_Innovus

setMultiCpuUsage -localCpu max
source Globals/I2CAndMemory_Test.globals
init_design
defIn ${SCAN_DEF_DIR}/I2CAndMemory.scan.def 
# get the scan.def file from genus
...
#add 3 pairs of stripes in layer 6 METTPL with widht 3 spacing 3 and offset from left and right 30.
...
#perform early global route from metals 6 to 2.
...
create_route_type -top_preferred_layer 4 -bottom_preferred_layer 3 -non_default_rule NDR_1 -name t_route -preferred_routing_layer_effort high -shield_net gnd -shield_side both_side
...
create_route_type -top_preferred_layer 4 -bottom_preferred_layer 3 -name l_route -preferred_routing_layer_effort high -shield_net gnd -shield_side both_side 
...
#======================Verify Antenna=======================================
verifyProcessAntenna -report ${REPORTS_DIR}/Post_NRoute/Verify_Post_NRoute/${MODE}_I2CAndMemory.antenna.rpt  -error 1000

saveDesign ${SAVES_DIR}/Top_${MODE}_5_Post_NRoute_Pre_MFill.enc
...
addMetalFill -layer { MET1 MET2 MET3 MET4 METTP METTPL } -net { vdd gnd } > ${REPORTS_DIR}/Post_NRoute/Verify_Post_NRoute/${MODE}_I2CAndMemory.Metal_Fill.rpt 
# -removeFloatingFill to avoid open metals violations from check conectivity.
...
verifyMetalDensity -report ${REPORTS_DIR}/Post_MFill/Verify_Post_MFill/${MODE}_I2CAndMemory.density.rpt

#====================Verify Drc Post MFill==========================

set_verify_drc_mode -disable_rules {} -check_ndr_spacing auto -check_only default -check_same_via_cell true -exclude_pg_net false -ignore_trial_route false -ignore_cell_blockage false -use_min_spacing_on_block_obs auto -report ${REPORTS_DIR}/Post_MFill/Verify_Post_MFill/${MODE}_I2CAndMemory.drc.rpt -limit 1000
verify_drc -report ${REPORTS_DIR}/Post_MFill/Verify_Post_MFill/${MODE}_I2CAndMemory.drc.rpt


#=====================Verify Connectivity Post MFill===========================

verifyConnectivity -type all -report ${REPORTS_DIR}/Post_MFill/Verify_Post_MFill/${MODE}_I2CAndMemory.conn.rpt -error 1000 -warning 50

#======================Verify Antenna Post MFill=======================================
verifyProcessAntenna -report ${REPORTS_DIR}/Post_MFill/Verify_Post_MFill/${MODE}_I2CAndMemory.antenna.rpt  -error 1000

report_power -output ${REPORTS_DIR}/Post_MFill/ -outfile ${REPORTS_DIR}/Post_MFill/${MODE}_MFill_power_WC.txt -view PVT_1_80_V_WC_VIEW
report_power -output ${REPORTS_DIR}/Post_MFill/ -outfile ${REPORTS_DIR}/Post_MFill/${MODE}_MFill_power_TYP.txt -view PVT_1_80_V_TYP_VIEW
report_power -output ${REPORTS_DIR}/Post_MFill/ -outfile ${REPORTS_DIR}/Post_MFill/${MODE}_MFill_power_BC.txt -view PVT_1_80_V_BC_VIEW
report_area > ${REPORTS_DIR}/Post_MFill/${MODE}_MFill_area.txt
report_timing > ${REPORTS_DIR}/Post_MFill/${MODE}_MFill_timing.txt

saveDesign ${SAVES_DIR}/Top_${MODE}_6_Post_MFill_Pre_Sign_Off.enc
...
extractRC
signoffTimeDesign -reportOnly -outDir ${REPORTS_DIR}/Sign_Off_Timing_Reports/

saveDesign ${SAVES_DIR}/Top_${MODE}_7_Post_Sign_Off.enc

#======================GDS OUT===============================

streamOut ${REPORTS_DIR}/GDS/Innovus_Out_GDS.gds -libName DesignLib -units 1000 -mode ALL
